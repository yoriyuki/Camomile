...
module Configure (Config : ConfigInt.Type) = struct
  ...
  let rec comp_sub_aux s1 i1 s2 i2 len =
    if len <= 0 then true
    else if s1.[i1] <> Bytes.get s2 i2 then false
    else comp_sub_aux s1 (i1 + 1) s2 (i2 + 1) (len - 1)

  let comp_sub s1 i1 len1 s2 i2 len2 =
    if len1 <> len2 then false else comp_sub_aux s1 i1 s2 i2 len1

  exception Malformed_code
  exception Out_of_range

  type 'a machine = { read : 'a -> unit; term : unit -> unit }
  ...
  let null_umachine = { read = (fun _ -> ()); term = (fun () -> ()) }
  ...
  let automatic name encs def =
    let make_decoder um =
      ...
      { read; term }
    in
    { name; make_decoder; make_encoder = def.make_encoder }
    ...
    let of_charmap charmap =
      ...
      let make_encoder cm =
        let read u =
          ...
          if enc = "" then raise Out_of_range else feed_string cm enc
        in
        let term () = cm.term () in
        { read; term }
      in
      { name = charmap.Charmap.name; make_decoder; make_encoder }
  end
  ...
  let char_machine_of outchan =
    ...
    let flush () =
      ...
      if n < !pos then begin
        pos := !pos - n;
        failwith "CharEncoding.char_machine_of: Cannot flush the entire buffer"
      end;
      outchan#flush ();
      pos := 0
    in
  ...
  class uchar_output_channel_of enc (output : char_output_channel) :
  ...
  class uchar_input_channel_of enc (input : char_input_channel) :
    ...
    object (self : 'a)
      ...
      method get () =
        try Queue.take q
        with Queue.Empty ->
          if term then raise End_of_file
          else (
            try
              let len = input#input b 0 1024 in
              for i = 0 to len - 1 do
                m.read (Bytes.get b i)
              done
            with End_of_file ->
              m.term ();
              term <- true);
    ...
    object
      ...
      method input s pos len =
        ...
        try
          ...
          !p - pos
        with End_of_file -> if eof then raise End_of_file else !p - pos
  ...
  module Make (Text : UnicodeString.Type) = struct
    ...
    let feed_text encoder s = Text.iter encoder.read s
  ...
  let _ =
    let make_decoder output_machine =
      ...
      let reader c =
        ...
        if code >= 0x80 then raise Malformed_code
        else (
          let u = UChar.chr_of_uint code in
          output u)
      in
      let term () = close () in
      { read = reader; term }
    ...
    let make_encoder output_machine =
      ...
      let reader u =
        ...
        if code < 0 || code >= 0x80 then raise Out_of_range
        else output (Char.chr code)
      in
      let term () = close () in
      { read = reader; term }
    in
    let enc () = { name = "US-ASCII"; make_decoder; make_encoder } in
    begin
  ...
  let _ =
    ...
    let make_encoder output_machine =
      ...
      let reader u =
        let c =
          try UChar.char_of u with Invalid_argument _ -> raise Out_of_range
        in
        output c
      in
      let term () = close () in
      { read = reader; term }
    in
    let enc () = { name = "Latin-1"; make_decoder; make_encoder } in
    begin
  ...
  type utf8_decoder_state = { mutable remain : int; mutable cur : int }
  ...
  let _ =
    ...
    let make_decoder output_machine =
      ...
      let reader c =
        ...
        if state.remain = 0 then
          (*beginning of char*)
          if i <= 0x7f then output (UChar.chr_of_uint i)
          else if i >= 0xc2 && i <= 0xdf then begin
            state.remain <- 1;
            state.cur <- i - 0xc0
          end
          else if i >= 0xe0 && i <= 0xef then begin
            state.remain <- 2;
            state.cur <- i - 0xe0
          end
          else if i >= 0xf0 && i <= 0xf7 then begin
            state.remain <- 3;
            state.cur <- i - 0xf0
          end
          else if i >= 0xf8 && i <= 0xfb then begin
            state.remain <- 4;
            state.cur <- i - 0xf8
          end
          else if i >= 0xfc && i <= 0xfd then begin
            state.remain <- 5;
            state.cur <- i - 0xfc
          end
          else raise Malformed_code
        else if i < 0x80 then raise Malformed_code
        else (
          let i' = i - 0x80 in
          let a = (state.cur lsl 6) + i' in
          (* reject unnecessary long encoding *)
          if a = 0 then raise Malformed_code
          else state.remain <- state.remain - 1;
          if state.remain = 0 then output (UChar.chr_of_uint a)
          else state.cur <- a)
      in
      let term () = close () in
      { read = reader; term }
    ...
    let make_encoder output_machine =
      ...
      let reader u =
        ...
        if k >= 0 && k <= 0x7f then output (Char.chr k)
        else if k >= 0x80 && k <= 0x7ff then (
      ...
      { read = reader; term }
    in
    let enc () = { name = "UTF-8"; make_decoder; make_encoder } in
    begin
      install "UTF-8" enc
    end
  ...
  type ucs2_machine = { read2 : int -> unit; term2 : unit -> unit }
  type ucs2_buf = { mutable first_char : bool; mutable buf : int }

  let char_machine_be m =
    let state = { first_char = true; buf = 0 } in
    let read c =
      if state.first_char then begin
        state.buf <- Char.code c;
  ...
  type utf16_decoder_state = { mutable surrogated : bool; mutable buf : int }
  ...
  let _ =
    ...
    let make_encoder m =
      ...
      let read u =
        ...
        if (i >= 0x0 && i < 0xd800) || (i >= 0xe000 && i < 0x10000) then
          output i
    ...
    let enc_unknown () = automatic "UTF-16" [enc_be; enc_le] enc_be in
  ...
  module UTF32 = struct
    type ucs4_machine = { read4 : int -> unit; term4 : unit -> unit }
    type ucs4_buf = { mutable count : int; mutable buf : int }

    let char_machine_be m =
      let state = { count = 0; buf = 0 } in
      let read c =
        let i = Char.code c in
        if state.count = 0 && i > 0x7f then raise Malformed_code
        else begin
    ...
    let char_machine_le m =
      ...
      let read c =
        let i = Char.code c in
        if state.count = 3 && i > 0x7f then raise Malformed_code
        else begin
    ...
    let make_ucs4_decoder m =
      let read n = m.read (UChar.chr_of_uint n) in
      let term () = m.term () in
    ...
    let make_utf32_encoder m =
      ...
      let read u =
        let n = UChar.uint_code u in
        if n > 0x10ffff || n < 0 || n = 0xfffe then raise Out_of_range
        else m.read4 n
      in
      let term () = m.term4 () in
      { read; term }

    let make_utf32_be_decoder m = char_machine_be (make_utf32_decoder m)
    let make_utf32_le_decoder m = char_machine_le (make_utf32_decoder m)
    let make_utf32_be_encoder m = make_utf32_encoder (be_outmachine m)
    let make_utf32_le_encoder m = make_utf32_encoder (le_outmachine m)
    ...
    let ucs4 =
      ...
      { name = "UCS-4"; make_decoder; make_encoder }
    ...
    let utf32_be =
      ...
      { name = "UTF-32BE"; make_decoder; make_encoder }

    (* Little endian order. *)
    let utf32_le =
      let make_decoder m = make_utf32_le_decoder m in
      let make_encoder m = make_utf32_le_encoder m in
      { name = "UTF-32LE"; make_decoder; make_encoder }

    (* UTF-32 with unknown endianness. *)
    let utf32 () = automatic "UTF-32" [utf32_be; utf32_le] utf32_be
  ...
  module Iso2022jp = struct
    ...
    let make_enc_2 () =
      ...
      let make_decoder_2 m =
        ...
        let reader c =
          let i = Char.code c in
          if i >= 0x80 then raise Malformed_code
          else if state.single_shift then
            if i >= 0x20 && i <= 0x7f then begin
            ...
            match state.remain with
              | 0 ->
                  (* escape sequence?. *)
                  if c = esc then state.remain <- -1
                  else if
                    (* control code or white space, backspace *)
                    i <= 0x20 || i = 0x7f
                  then m.read (UChar.chr_of_uint i)
                  else if unibyte state.g0 then m.read (to_uchar_2 state.g0 i)
                  else begin
                    state.remain <- 1;
                    Bytes.set state.buf 0 c
                  end
              | 1 ->
                  let i1 = Char.code (Bytes.get state.buf 0) in
                  let i2 = Char.code c in
                  if i2 >= 0x21 && i2 <= 0x7e then (
              ...
              | _ ->
                  ...
                  if i >= 0x20 && i <= 0x2f then
                    ...
                    state.remain <- 0;
                    if comp_sub "(B" 0 2 state.buf 0 len then state.g0 <- Ascii
      ...
      let make_encoder_2 m =
        let from_uchar_2 u =
          ...
          if n >= 0x00 && n <= 0x7f then (Ascii, n)
          else if n >= 0xa0 && n <= 0xff then (Iso88591, n - 0x80)
          else (
            let enc = Unimap.ucs_to_enc jisx0208 n in
            if enc <> Unimap.no_char_enc jisx0208 then (Jisx0208_1983, enc)
            else (
              let enc = Unimap.ucs_to_enc jisx0212 n in
              if enc <> Unimap.no_char_enc jisx0212 then (Jisx0212_1990, enc)
              else (
                let enc = Unimap.ucs_to_enc gb2312 n in
                if enc <> Unimap.no_char_enc gb2312 then (Gb2312_1980, enc)
                else (
                  let enc = Unimap.ucs_to_enc ksc5601 n in
                  if enc <> Unimap.no_char_enc ksc5601 then (Ksc5601_1987, enc)
                  else (
                    let enc = Unimap.ucs_to_enc iso88597 n in
                    if enc <> Unimap.no_char_enc iso88597 then
                      (Iso88597, enc - 0x80)
                    else (
                      let n' = Unimap.ucs_to_enc jisx0201 n in
                      if n' = Unimap.no_char_enc jisx0201 then
                        raise Out_of_range
                      else if n' >= 0x80 then (Jisx0201_kana, n' - 0x80)
                      else (Jisx0201_roman, n')))))))
        in
        ...
        let reader u =
          ...
          if c1 = esc || c2 = esc then raise Out_of_range
          else if set94 cs then
            if state.g0 = cs then
              if unibyte cs then m.read c2
              else begin
                m.read c1;
                m.read c2
              end
    ...
    let make_enc () =
            ...
            match state.remain with
              | 0 ->
                  ...
                  if c = esc then state.remain <- -1
                    ...
                    i <= 0x20 || i = 0x7f
                  then m.read (UChar.chr_of_uint i)
                  else if unibyte state.g0 then m.read (to_uchar state.g0 i)
                  else begin
                    state.remain <- 1;
                    Bytes.set state.buf 0 c
                  end
      ...
      let make_encoder m =
        let from_uchar u =
          ...
          if n >= 0x00 && n <= 0x7f then (Ascii, n)
            ...
            if enc <> Unimap.no_char_enc jisx0208 then (Jisx0208_1983, enc)
              ...
              let n' = Unimap.ucs_to_enc jisx0201 n in
              if n' = Unimap.no_char_enc jisx0201 then raise Out_of_range
              else if n' >= 0x80 then raise Out_of_range
              else (Jisx0201_roman, n')))
        in
        (*fields other than g0, g2 are not used*)
        let state =
        ...
        in
        let reader u =
          if UChar.uint_code u = 27 then raise Out_of_range
          else (
      ...
      { name = "ISO-2022-JP"; make_decoder; make_encoder }
  ...
  module Iso2022kr = struct
    ...
    let make_enc () =
      ...
      let make_decoder m =
        let state = { remain = 0; gl = Ascii; buf = Bytes.create 3 } in
        let reader c =
          let i = Char.code c in
          if i >= 0x80 then raise Malformed_code
          else (
            match state.remain with
              | 0 ->
                  if c = esc then state.remain <- -1
                  else if c = si then state.gl <- Ascii
                  else if c = so then state.gl <- Ksc5601
                  else if
                    (* control code or white space, backspace *)
                    i <= 0x20 || i = 0x7f
                  then m.read (UChar.chr_of_uint i)
      ...
      let make_encoder m =
        ...
        let state = { remain = 0; gl = Ascii; buf = Bytes.create 1 } in
        feed_string m "\027$)C";
        (*designator*)
        let reader u =
          let n = UChar.code u in
          if n <= 0x7f then
            if n = 0x0e || n = 0x0f || n = 0x1b then raise Out_of_range
            else (
              match state.gl with
            ...
            if i = Unimap.no_char_enc ksc5601 then raise Out_of_range
            else (
              let c1 = Char.chr (i lsr 8) in
              let c2 = Char.chr (i land 255) in
              match state.gl with
      ...
      { name = "ISO-2022-KR"; make_decoder; make_encoder }
  ...
  module Iso2022cn = struct
    ...
    let make_enc () =
          ...
          if i >= 0x80 then raise Malformed_code
            ...
            match state.remain with
              | 0 ->
                  if not state.single_shift then (
                    match c with
                      ...
                      | _ ->
                          (* control code *)
                          if i <= 0x20 then m.read (UChar.chr_of_uint i)
                          else (
                            match state.gl with
              ...
              | 1 ->
                  ...
                  let cs =
                    if state.single_shift then state.g2
                    else (
                      match state.gl with
                        | G0 -> Ascii
                        | G1 -> state.g1
              ...
              | _ ->
                  ...
                  if i >= 0x20 && i <= 0x2f then
                    ...
                    else if comp_sub "N" 0 1 state.buf 0 len then begin
                      state.single_shift <- true;
                      state.remain <- 2
                    end
                    else raise Malformed_code
      ...
      let make_encoder m =
        let from_ucs4 n =
          if n <= 0x7f then (Ascii, n)
          else (
            let n' = Unimap.ucs_to_enc gb2312 n in
            if n' <> Unimap.no_char_enc gb2312 then (GB_2312, n')
            else (
              let n' = Unimap.ucs_to_enc cns11643 n in
              if n' = Unimap.no_char_enc cns11643 then raise Out_of_range
              else (
                match n' lsr 16 with
      ...
      { name = "ISO-2022-CN"; make_decoder; make_encoder }
