...
let () =
  test ~desc:"Output Test" ~body:(fun () ->
      expect_pass ~body:(fun () ->
          ...
          let c = new char_obj_output_channel_of (bio :> char_output_channel) in
          let c = new char_output_channel_of c in
          let put c s =
            ignore (c#output (Bytes.of_string s) 0 (String.length s))
          in
          put c "---------------\n";
          c#flush ();
...
let () =
  test ~desc:"channel_of_stream test" ~body:(fun () ->
      expect_pass ~body:(fun () ->
          ...
          (try
             while true do
               Buffer.add_char b (c#get ())
             done
           with End_of_file -> ());
          expect_equal
            ~msg:(lazy (sprintf "output differ"))
            (Buffer.contents b) text))
...
class otext =
  object
    ...
    method get () =
      if counter >= String.length text then raise End_of_file
      else begin
...
let () =
  test ~desc:"stream_of_channel test" ~body:(fun () ->
      expect_pass ~body:(fun () ->
...
let () =
  test ~desc:"char_input_channel_of test" ~body:(fun () ->
      expect_pass ~body:(fun () ->
          ...
          expect_equal
            ~msg:
              (lazy (sprintf "length %d should be %d" n (String.length text)))
            n (String.length text);
          expect_equal ~msg:(lazy "output differ") (Bytes.to_string b) text;
          expect_equal_app
            ~msg:(lazy "EOF")
...
let () =
  test ~desc:"char_obj_input_channel_of test" ~body:(fun () ->
      expect_pass ~body:(fun () ->
          let occ = new char_obj_input_channel_of (new otext_char) in
          let b = Buffer.create 0 in
          (try
             while true do
               Buffer.add_char b (occ#get ())
             done
           with End_of_file -> ());
          expect_equal
            ~msg:(lazy (sprintf "output differ"))
