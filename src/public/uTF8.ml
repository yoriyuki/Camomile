...
let look s i =
  let n' =
    ...
    if n < 0x80 then n
    else if n <= 0xdf then ((n - 0xc0) lsl 6) lor (0x7f land Char.code s.[i + 1])
    else if n <= 0xef then (
      let n' = n - 0xe0 in
...
let rec search_head_backward s i =
  if i < 0 then -1
    ...
    let n = Char.code s.[i] in
    if n < 0x80 || n >= 0xc2 then i else search_head_backward s (i - 1))
...
let add_uchar buf u =
  ...
  else if k <= 0x7f then Buffer.add_char buf (Char.unsafe_chr k)
...
let validate s =
  let rec trail c i a =
    if c = 0 then a
    else if i >= String.length s then raise Malformed_code
    else (
      let n = Char.code (String.unsafe_get s i) in
      if n < 0x80 || n >= 0xc0 then raise Malformed_code
      else trail (c - 1) (i + 1) ((a lsl 6) lor (n - 0x80)))
  in
  let rec main i =
    if i >= String.length s then ()
    else (
      let n = Char.code (String.unsafe_get s i) in
      if n < 0x80 then main (i + 1)
      else if n < 0xc2 then raise Malformed_code
      else if n <= 0xdf then
        if trail 1 (i + 1) (n - 0xc0) < 0x80 then raise Malformed_code
        else main (i + 2)
      else if n <= 0xef then
        if trail 2 (i + 1) (n - 0xe0) < 0x800 then raise Malformed_code
        else main (i + 3)
      else if n <= 0xf7 then
        if trail 3 (i + 1) (n - 0xf0) < 0x10000 then raise Malformed_code
        else main (i + 4)
      else if n <= 0xfb then
        if trail 4 (i + 1) (n - 0xf8) < 0x200000 then raise Malformed_code
        else main (i + 5)
      else if n <= 0xfd then (
        let n = trail 5 (i + 1) (n - 0xfc) in
        if n lsr 16 < 0x400 then raise Malformed_code else main (i + 6))
