...
let eq t1 t2 =
  if UText.length t1 <> UText.length t2 then false else
  let rec loop i =
    if i >= UText.length t1 then true else
    if UText.get t1 i = UText.get t2 i then loop (i + 1) else
    false
  in
  loop 0
...
let _ = read_file (input_filename testfile) (fun c ->
  let is_part1 = ref false in
  try while true do
    let line = input_line c in
    if Str.string_match comment line 0 then () else
    if Str.string_match part line 0 then
      is_part1 := false
    else if Str.string_match part1 line 0 then
      is_part1 := true
    else
      ...
      (* NFC test *)
      test ~desc:"NFC" ~body:(fun () -> expect_pass (fun () ->
	let nfc = Array.map NF.nfc c in
	expect_true
	  ~msg:(lazy (sprintf "c2 %s <> NFC(c1) %s" 
		  (print_text c.(1)) 
		  (print_text nfc.(0))))
	  (eq c.(1) nfc.(0));
      	expect_true
	  ~msg:(lazy (sprintf "c2 %s <> NFC(c2) %s" 
		  (print_text c.(1)) 
		  (print_text nfc.(1))))
	  (eq c.(1) nfc.(1));
	expect_true
	  ~msg:(lazy (sprintf "c2 %s <> NFC(c3) %s" 
		  (print_text c.(1)) 
		  (print_text nfc.(2))))
	  (eq c.(1) nfc.(2));
	expect_true
	  ~msg:(lazy (sprintf "c4 %s <> NFC(c4) %s" 
		  (print_text c.(3)) 
		  (print_text nfc.(3))))
	  (eq c.(3) nfc.(3));
	expect_true
	  ~msg:(lazy (sprintf "c4 %s <> NFC(c5) %s" 
		  (print_text c.(3)) 
		  (print_text nfc.(4))))
	  (eq c.(3) nfc.(4))));

      (* NFD test *)
      test ~desc:"NFD" ~body:(fun () -> expect_pass (fun () ->
	let nfd = Array.map NF.nfd c in
	expect_true
	  ~msg:(lazy (sprintf "c3 %s <> NFD(c1) %s" 
		  (print_text c.(2)) 
		  (print_text nfd.(0))))
	  (eq c.(2) nfd.(0));
      	expect_true
	  ~msg:(lazy (sprintf "c3 %s <> NFD(c2) %s" 
		  (print_text c.(2)) 
		  (print_text nfd.(1))))
	  (eq c.(2) nfd.(1));
	expect_true
	  ~msg:(lazy (sprintf "c3 %s <> NFD(c3) %s" 
		  (print_text c.(2)) 
		  (print_text nfd.(2))))
	  (eq c.(2) nfd.(2));
	expect_true
	  ~msg:(lazy (sprintf "c5 %s <> NFD(c4) %s" 
		  (print_text c.(4)) 
		  (print_text nfd.(3))))
	  (eq c.(4) nfd.(3));
	expect_true
	  ~msg:(lazy (sprintf "c5 %s <> NDF(c5) %s" 
		  (print_text c.(4)) 
		  (print_text nfd.(4))))
	  (eq c.(4) nfd.(4))));

      (* NFKC test *)
      test ~desc:"NFKC" ~body:(fun () -> expect_pass (fun () ->
	let nfkc = Array.map NF.nfkc c in
	expect_true
	  ~msg:(lazy (sprintf "c4 %s <> NFKC(c1) %s" 
		  (print_text c.(3)) 
		  (print_text nfkc.(0))))
	  (eq c.(3) nfkc.(0));
      	expect_true
	  ~msg:(lazy (sprintf "c4 %s <> NFKC(c2) %s" 
		  (print_text c.(3)) 
		  (print_text nfkc.(1))))
	  (eq c.(3) nfkc.(1));
	expect_true
	  ~msg:(lazy (sprintf "c4 %s <> NFKC(c3) %s" 
		  (print_text c.(3)) 
		  (print_text nfkc.(2))))
	  (eq c.(3) nfkc.(2));
	expect_true
	  ~msg:(lazy (sprintf "c4 %s <> NFKC(c4) %s" 
		  (print_text c.(3)) 
		  (print_text nfkc.(3))))
	  (eq c.(3) nfkc.(3));
	expect_true
	  ~msg:(lazy (sprintf "c4 %s <> NFKC(c5) %s" 
		  (print_text c.(3)) 
		  (print_text nfkc.(4))))
	  (eq c.(3) nfkc.(4))));

      (* NFKD test *)
      test ~desc:"NFKD" ~body:(fun () -> expect_pass (fun () ->
	let nfkd = Array.map NF.nfkd c in
	expect_true
	  ~msg:(lazy (sprintf "c5 %s <> NFKD(c1) %s" 
		  (print_text c.(4)) 
		  (print_text nfkd.(0))))
	  (eq c.(4) nfkd.(0));
      	expect_true
	  ~msg:(lazy (sprintf "c5 %s <> NFKD(c2) %s" 
		  (print_text c.(4)) 
		  (print_text nfkd.(1))))
	  (eq c.(4) nfkd.(1));
	expect_true
	  ~msg:(lazy (sprintf "c5 %s <> NFKD(c3) %s" 
		  (print_text c.(4)) 
		  (print_text nfkd.(2))))
	  (eq c.(4) nfkd.(2));
	expect_true
	  ~msg:(lazy (sprintf "c5 %s <> NFKD(c4) %s" 
		  (print_text c.(4)) 
		  (print_text nfkd.(2))))
	  (eq c.(4) nfkd.(3));
	expect_true
	  ~msg:(lazy (sprintf "c5 %s NKKD(c5) %s" 
		  (print_text c.(4)) 
		  (print_text nfkd.(4))))
	  (eq c.(4) nfkd.(4))));
...
let _ = 
  ...
  for i = 0 to 1 lsl 21 - 1 do
    ...
    match cat u with `Co | `Cn -> () | _ ->
      if S.mem u !uset then () else
